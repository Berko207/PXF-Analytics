...
_BOUT_HEADER = re.compile(
    r"^(?:(?P<number>\d+)\.\s*)?"
    r"(?:(?P<label>main event|co-main(?: event)?|featured bout|preliminary bout)\s*[-–—]\s*)?"
    r"(?P<weight>[\w\s]+)?"
    r"(?P<title>\s*\(title\))?\s*$",
    re.IGNORECASE,
)
...
def parse_fight_card(text: str) -> RawFightCard:
    ...
    bout_number = 0
    while index < len(lines):
        bout_header = _BOUT_HEADER.match(lines[index])
        if not bout_header:
            index += 1
            continue
        ...
        weight_class = (bout_header.group("weight") or "").strip() or None
        ...
        while index < len(lines):
            if _BOUT_HEADER.match(lines[index]):
                break
